...
const Allmapping = () => {
  ...
  useEffect(() => {
    fetchTheme();
    handeldata();
    axios.get("http://localhost:4040/skill/findall").then(res => {
  ...
  const handeldata = async () => {
    try {
      const result = await axios.get("http://localhost:4040/mapping/findall");
      setShow(result.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handeldelete = async (id) => {
    try {
      const result = await axios.delete(`http://localhost:4040/mapping/delete/${id}`);
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      await axios.patch(`http://localhost:4040/mapping/update/${id}`, payload);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post("http://localhost:4040/mapping/", payload);
      ...
      {open && (
        <div className="fixed inset-0 flex items-center justify-center  bg-opacity-50 z-50">
          ...
          <div className={`relative p-6 rounded-xl shadow-xl w-[90%] max-w-md space-y-4 ${cardThemeClasses}`}>
            <h2 className="text-2xl font-bold">Skill & Course Mapping</h2>
            ...
            <div>
              ...
              <div className={`border rounded p-2 flex flex-wrap gap-2 ${borderClasses}`}>
                {selectedSkills.map(skill => (
                  <span
                    key={skill._id}
                    className={`${chipClasses.skill} px-2 py-1 rounded flex items-center gap-1`}
                  >
                    {skill.name}
                  ...
                  <td className="border px-4 py-2">
                    <div className="flex flex-wrap gap-1">
                      {x.skills?.map(s => (
                        <span
                          key={s._id}
                          className={`inline-block ${chipClasses.skill} px-2 py-1 rounded text-sm`}
                        >
                          {s.name}
                        </span>
                      ))}
            ...
            <div className={`border rounded p-2 flex flex-wrap gap-2 mb-3 ${borderClasses}`}>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setEditData({
                        ...editData,
                        skills: editData.skills.filter(s => s._id !== skill._id)
                      })
                    }
                    className="font-bold"
                  >×</button>
              ...
              <select
                onChange={(e) => {
                  ...
                  if (selected && !editData.skills?.find(s => s._id === selected._id)) {
                    setEditData({
                      ...editData,
                      skills: [...(editData.skills || []), selected]
                    });
                  }
                }}
